...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        expressions = Pair(expressions.first.rest, expressions.rest)
        env.define(signature, do_lambda_form(expressions, env))
        return signature
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while True:
        ...
        if is_scheme_false(value) or expressions.rest is nil:
            return value
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        name, val = pair.first, scheme_eval(pair.rest.first, env)
