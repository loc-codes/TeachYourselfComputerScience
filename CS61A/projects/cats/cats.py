...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    selected_paragraphs = [paragraph for paragraph in paragraphs if select(paragraph)]
    ...
    def helper(paragraph):
        ...
        for word in subject:
            for string in paragraph:
                word, string = remove_punctuation(lower(word)), remove_punctuation(lower(string))
                if word == string:
                    return True
        return False
    return helper
    ...
    for word in word_list:
        if word == typed_word:
            ...
            if difference < min_difference and difference <= limit:
                corrected_word, min_difference = word, difference
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]:  # Feel free to remove or add additional cases
        ...
        add = lambda typed, source: source[0] + typed  # Fill in these lines
        remove = lambda typed: typed[1:]
        substitute = lambda typed, source: source[0] + typed[1:]
        # BEGIN
        #Case 1: Add
        add_case = 1 + minimum_mewtations(add(typed,source), source,limit-1)
        #Case 2: Remove
        remove_case = 1 + minimum_mewtations(remove(typed),source,limit-1)
        #Case 3: Substitute
        substitute_case = 1 + minimum_mewtations(substitute(typed,source),source,limit-1)
        return min(add_case,remove_case,substitute_case)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    upload_dict = {'id': user_id, 'progress': progress}
    upload(upload_dict)
    return progress
    # END PROBLEM 8
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    answer = [[] for player_index in player_indices]
    for word_index in word_indices:
        min_time = 99999999
        for player_index in player_indices:
            typing_time = time(match,player_index,word_index)
            if typing_time < min_time:
                min_time,winning_player = typing_time,player_index
        answer[winning_player].append(get_word(match,word_index))
    return answer
