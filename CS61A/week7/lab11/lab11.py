...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        return exp
    elif bindings[exp]:  # CHANGE THIS CONDITION FOR Q4
...
def floor_div(expr):
    ...
    2
    >>> floor_div(Pair(80, Pair(10, Pair(3, Pair(2, nil)))))
    1
    ...
    answer = expr.first
    while expr.rest != nil:
        answer //= expr.rest.first
        expr.rest = expr.rest.rest
    return answer
...
def eval_and(operands):
    ...
    if operands == nil:
        ...
        while operands != nil:
            value = calc_eval(operands.first)
            if not value and not value is 0:
                return False
            else:
                truthy_value, operands = value, operands.rest
        return truthy_value
